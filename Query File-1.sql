...
WHERE
    table_schema = 'imdb' order by Row_Count desc ;

-- Total number of rows in  each table of the schema are displayed.
...
-- Four columns in the movie table contain null values. Now, let's examine the movies released each year.
...
-- number of movies produced in the year 2019 are 887
...
-- higest number of movies produced overall is in Drama genre
...
-- number of movies present in only one genre is 3289
...
(
	SELECT genre, COUNT(movie_id) AS movie_count,
			RANK() OVER(ORDER BY COUNT(movie_id) DESC) AS thriller_genre_rank
	FROM genre
	GROUP BY genre
)
...
-- the rank of the ‘thriller’ genre of movies among all the genres is 3
...
-- Type your code below:
SELECT 
    MIN(avg_rating) AS lowest_avg_rating, 
    MAX(avg_rating) AS highest_avg_rating,
    MIN(total_votes) AS lowest_total_votes, 
    MAX(total_votes) AS highest_total_votes,
    MIN(median_rating) AS lowest_median_rating, 
    MAX(median_rating) AS highest_median_rating
FROM ratings;
-- The minimum and maximum values in  each column of the ratings table
...
-- This is the list of top 10 movies based on their average rating.
...
-- Production house list with most number of hit movies
-- It's ok if RANK() or DENSE_RANK() is used too
...
-- Movies released in each genres in march 2017
...
-- movies released between 1 April 2018 and 1 April 2019 with 8 rating
...
WHERE 
    languages LIKE '%German%' OR languages LIKE '%Italian%'
GROUP BY 
    languages
ORDER BY 
    highest_total_votes DESC;

-- German movies get more vites than Italian
-- Answer is Yes
...
top_directors AS (
    ...
    JOIN 
        top_genres tg ON g.genre = tg.genre
    WHERE 
        r.avg_rating > 8
    GROUP BY 
        n.name
...
SELECT 
    ...
    ROUND(SUM(r.avg_rating * r.total_votes) / SUM(r.total_votes), 2) AS actress_avg_rating,
    DENSE_RANK() OVER(ORDER BY COUNT(DISTINCT m.id) DESC, SUM(r.total_votes) DESC) AS actress_rank
FROM 
    names AS nm
INNER JOIN 
    role_mapping AS rm ON nm.id = rm.name_id
INNER JOIN 
